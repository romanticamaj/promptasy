...
- [ ] **P06c · 把 7 個空區的路填滿（閘門 A 回饋 ②）**（M–L，可拆 a/b）— 站長實玩裁決：**「顏色我看不出來、路上的物件要多一點」**。查證屬實且更嚴重：課程 v2 後加的 **7 區（forms／toolcraft／wards／refinery／frugality／sight／divergence）反應物 0、器物 0**——原本 5 區每區 8–12 件。用 P06 的節奏稽核當靶，把 14 種**現成**的種類（風鈴架／光菇圈／音石／水紋池／守望的小獸／螢火；罐／火盆／響石／守望石／撈月池／指路石／絞盤／長凳）依各區調性補進去；順帶讓 P06 的區域色（key／rim／particle）**終於有東西可以顯色**。
