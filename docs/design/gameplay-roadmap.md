...
- [ ] **P22b · 畫面成本修復（draw call／透明片／材質進預算）**（M）— `scripts/perf-audit.mjs` 進 `npm run audit:perf` 並寫進 `expected-counts.json` 契約（逐值、先紅後綠）；石座層（852 draw／852 材質／426 加色）共用材質與幾何、看不見的光暈不進渲染佇列；小景層（712 draw）依距離分帶；**低畫質要真的省 draw call**（現在只省 89 個，卻砍掉 6 萬個三角 —— 優化到了不是瓶頸的那一項）。
  - DoD：高畫質 draw call 較 4,144 **降 ≥ 25%**、加色混合 **降 ≥ 30%**；低畫質再比高畫質少 ≥ 20% draw；畫面逐項比對無可見差異；契約有斷言、拆掉優化會紅。
- [ ] **P22c · 世界攤開（地圖放大 ×1.3）**（L）— `WORLD_RADIUS` 150 → 195、地形平面 ±170 → ±215，**座標乘 k、尺寸與互動半徑不乘** → 密度降 1.69 倍；區心／區徑／走廊／捷徑／中景帶／高台／小景／石座／濁靈／守夜人／殘頁／祕密／檔案廊逐表遷移；四支 `--verify` 與 `audit:pacing`／`audit:sightline` 全綠。
