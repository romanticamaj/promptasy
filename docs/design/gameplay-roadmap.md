...
- [~] **P16d · 走不走得到要看坡度（土地邊緣不再走得下懸崖）**（M）— 站長實玩後追加：`isWalkable()` 只問 `coverage ≥ 0.45`，而 coverage 是**水平**混合值、與高度場無關 —— 實測玩家可以走出土地邊緣 5.5 公尺、**往下走 19–21 公尺的崖面**再走回來（全地圖 22.5% 的可走地形陡於 30°，3,918 個取樣點是 70–90° 的垂直面）。加**斜度上限**（≤35°，橋的甲板最陡才 13°，不受影響）；四片加建院落的**頸口本身是垂直溜滑梯**，要真的做成走得上去的坡。
