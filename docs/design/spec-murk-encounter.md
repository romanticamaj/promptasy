...
> **分支**：`feature/tainted-request-encounter` ｜ **成文**：2026-08-17 ｜ **狀態**：草稿，待站長核准後開工
...
5. **達標**（`score ≥ pass`）：最後一層殼散開，眼光轉為暖白，濁靈縮成一顆小光靈；`hud.celebrate`／既有過關音；記入圖鑑。關掉面板後小光靈**跟著玩家漂 ~8 秒**（半徑 2m、只跟不擋、不進碰撞），然後飄回原位落成一盞小燈（安撫後的濁靈＝該處的常駐光點，回頭看得到自己安撫過的痕跡）。
6. **重玩**：安撫過的濁靈仍可按 E 再寫（拿更高評價），與石座 S/A/B/C 同邏輯。
...
- 新增回呼 `onRubricHits?.({ challenge, hits:[{check, passed, weight}] })` 在評分完成、顯示結果**之前**觸發（給世界做逐殼演出）；既有 `onResult` 不變。

### 4.4 進程與存檔
- `save.js`：新增 `murksCalmed: []`（`defaultSave` ＋ `normalize` 的 `[...new Set(strArr(...))]`，純加法，`reset()` 自然清除），最佳評價存在既有 `bestGrades[murkId]`（避免另開 map；`expected-counts` 的 challenges 契約不受影響，因為 murk 不進 `content.challenges`）。
- `progression.js`：`calmMurk(id, evaluation)` → 記 `murksCalmed`、`bestGrades`、XP（`murks.json.xp`，只補差額，與 `recordResult` 同規則）、`newlyCollected` 沿用 teaches；`murkCount()`。**不影響 `refreshUnlocks()`**（WORLD.md:723-730 規範）。
...
- **`npm run test:playtest`** 新增：每隻 `sample` ≥ A；`taint` 本身送進去必不過（弱起手必不過）。
- **`npm run test:e2e`** 新增（鏡像 `headless-check.mjs:1849-1866` 與 `2437-2481`）：teleport 到某隻濁靈旁 → `[data-interact]` 顯示「濁靈…安撫」→ `KeyE` → `promptConsole.isOpen` 且模式為 free、第一幕含 taint 文字 → 送 taint 原文 → 未安撫、殼數不變、無 XP → 送 sample → `murksCalmed` 含 id、`bestGrades[id]` 有值、殼數 0、圖鑑第四列 `1/8`；`Escape` 後 8 秒內光靈跟隨、之後回原位；tris/lights 預算斷言沿用；console error 0。動畫時序斷言一律**輪詢式**。
- **`npm run build`**、**`npm run fonts`**（指紋測試會攔）。
- 手動：走一遍 8 隻，確認提示、演出、音效、圖鑑；WORLD.md 維護檢查表逐條過。

## 7. 完成定義（DoD）

- [ ] 8 隻濁靈可見、可互動、可安撫、可重玩、進圖鑑。
...
估 **M–L**（Codex 上修後的估計）。建議一個 subagent 一次做完（單一 phase、單一 brief），禁區：`curriculum.json`、`challenges.json`、`flows.json`、`vite.config.js`、使用者的 dev server（5175）、`CLAUDE.md`（changelog 由 orchestrator 補）。
